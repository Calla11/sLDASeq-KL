function [ geneExp ] = Expression( alpha, refFlat_line, beta, lineNo, readNum)
...
isoLen = load([str_temp,'/workfloder/','isoLen/',char(refFlat_line{1}(1))]);
resultDir = [];
readNo= [str_temp,'/workfloder/','ModelMultiGene_Data/',char(refFlat_line{1}(1))];
...
expressionFile = ['./Expression_gene/',char(refFlat_line{1}(1))];
...
f_iso= fopen(['./','Expression_iso.txt'], 'a');
for i=1:iso_No
