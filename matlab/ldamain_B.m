function [alpha,beta] = ldamain_B(emmax,demmax)
global   k
...
while ~feof(fid)
  ...
  if (str2num(char(refFlat_line{1}(2))) > 1)
      ...
      a = [str_temp,'/workfloder_B/','ModelMultiGene_Data/',char(refFlat_line{1}(1))];
      a1 = [str_temp,'/workfloder_B/','ModelMultiGene_Map/',char(refFlat_line{1}(1))];
      f=fopen(a);
      f1=fopen(a1);
      if f>0&&f1>0
          al=['./alpha_B/',char(refFlat_line{1}(1))];
          f_alpha=fopen(al);
          if f_alpha>0
              fclose(f_alpha);
          else
              d = fmatrix(a);
              lineNo = size(d,2);
              beta = load([str_temp,'/workfloder_B/','ModelMultiGene_Map/',char(refFlat_line{1}(1))]);
              if size(beta,2) ~=1
                  n = length(d); 
                  count=0;
                  for i=1:n
                  ...
                  if count~=0
                      ...
                      dlmwrite(['./alpha_B/',char(refFlat_line{1}(1))],alpha/sum(alpha));
                      dlmwrite(['./eta_B/',char(refFlat_line{1}(1))],eta/sum(eta));
                      Expression_B(alpha, refFlat_line, beta, lineNo, readNum);
                  else
                      continue
                  end
              else
